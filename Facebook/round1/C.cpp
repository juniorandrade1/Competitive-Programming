...
ll func(int p1, int p2, int op) {
  ...
  if(p2 < q && p2 - p1 <= 1) ret = min(ret, func(p1, p2 + 1, 0) + g[x][st[p2]]);
  if(p1 < p2) ret = min(ret, func(p1 + 1, p2, 1) + g[x][ed[p1]]);
...
inline void main2() {
  ...
  q++;
  for(int i = 0; i <= q; ++i) for(int j = 0; j <= q; ++j) pd[i][j][0] = pd[i][j][1] = -1;
  ll ans = func(1, 2, 0) + g[0][st[1]];
  if(ans >= LINF) ans = -1;
  printf(" %lld\n", ans);
}
