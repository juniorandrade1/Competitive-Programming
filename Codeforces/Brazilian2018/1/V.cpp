...
int main() {
  ...
  for(int test = 0; test < t; ++test) {
    ...
    scanf("%lf %lf %lf", &ax, &ay, &ar);
    scanf("%lf %lf %lf", &bx, &by, &br);
    PT a = PT(ax, ay);
    PT b = PT(bx, by);
    vector< PT > sol = CircleCircleIntersection(a, b, ar, br);
    if(sol.size() == 0) {
      long double d = sqrt(dist2(a, b));
      if (d > ar+br) puts("There are no points");
      else puts("Oops, too many points");
    }
    else {
      sort(sol.begin(), sol.end());
      printf("There are only %d of them\n", (int)sol.size());
      for(int i = 0; i < (int)sol.size(); ++i) {
        printf("%.6lf %.6lf\n", (double)sol[i].x, (double)sol[i].y);
      }
    }
  }
  return 0;
