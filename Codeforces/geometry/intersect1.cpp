...
struct Line {
  ...
  Line(ld _a, ld _b, ld _c) {
    ...
    ld x0 = (rand() % 10000000 + 1) / 10000000.;
    ld y1 = (rand() % 10000000 + 1) / 10000000.;
    Point s, t;
...
int main() {
 ...
 // freopen("intersec1.out", "w", stdout);
  ...
  /*
  ld x = ((ld)(c1 * b2) - (ld)(c2 * b1)) / ((ld)(a2 * b1) - (ld)(a1 * b2));
  ld y = 0;
  if(fabs(b1) > eps) y = ((ld)(-c1 - a1 * x) / (ld)b1);
  else y = ((ld)(-c2 - a2 * x) / (ld)b2);
  printf("%.10lf %.10lf\n", (double)x, (double)y);
  */
  return 0;
}