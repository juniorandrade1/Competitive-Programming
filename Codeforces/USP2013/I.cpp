...
private:
  const CTYPE COSTINF = numeric_limits<FTYPE>::max() / 2.0; 
  const FTYPE FLOWINF = numeric_limits<CTYPE>::max() / 2.0;
...
inline void main2() {
  ...
  int sta = n, fin = n + 1;
  MinCostMaxFlow<ll, ll> flowGraph(n + 2, sta, fin);
  int needFlow = 0;
  for(int x = 0; x < n; ++x) {
